...
function _registerListener(win, opts = {}) {
    ...
    const listener = (e, item) => {
        ...
        const itemFilename = customFileNameGlobal;
...
const download = (options, callback) => {
    ...
    request.on('response', function (response) {
        ...
        customFileNameGlobal = options.customFileName
        console.log("the request at 164 got this filename: " + filename)
        ...
        if (fs.existsSync(filePath)) {
            ...
            win.webContents.downloadURL(options.url, filename);
        }
    });
    request.end();
};
